    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard
          ...
          delta="+18%"
          deltaPositive
          sparkData={sessions.map((s) => ({ v: s.v }))}
          sparkType="area"
        />
        <StatCard
          ...
          delta="+12%"
          deltaPositive
          sparkData={sessions.map((s) => ({ v: s.v * 0.66 }))}
          sparkType="area"
        />
        <StatCard label="Bounce rate" value="44%" delta="-3%" deltaPositive />
        <StatCard
          label="Estimator start rate"
          value="24.5%"
          delta="+2.1%"
          deltaPositive
        />
      ...
      {/* Estimator funnel */}
      <ChartCard
        title="Estimator funnel"
        subtitle="Drop-off at each step"
        chartHeight={200}
      >
          ...
          {funnel.map((step, i) => {
            ...
            return (
              <div key={step.name} className="flex items-center gap-3">
                <span className="w-38 shrink-0 text-[12px] text-slate-400">
                  {step.name}
                </span>
          ...
          <table className="w-full text-[13px]">
            ...
            <tbody>
              {topPages.map((p) => (
                ...
                >
                  <td className="py-2.5 pr-4 font-mono text-[12px] text-slate-300">
                    {p.path}
                  </td>
